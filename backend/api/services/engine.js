...
async function processVerify(transRefId, credential, ctx) {
  ...
  try {
    ...
    const safe = await scrubSecuredTrailData(service, body, trail0.inputMessage);
    await TransactionTrail.updateOne(transRefId).set({ inputMessage: safe.inputMessage, outputMessage: { TRANSBODY: safe.body } });
    ...
    const safe = await scrubSecuredTrailData(service, body, trail0.inputMessage);
    await TransactionTrail.updateOne({ id: transRefId, status: 'processing' }).set({ status: 'failed', inputMessage: safe.inputMessage, outputMessage: { TRANSBODY: safe.body } });
    throw e;
  } finally {
