...
| **Sổ sách**  | `Pocket` | `ownerType` (customer/system/bank/biller), `ownerRef`, `balance`, `checksum`, `state` (idle/inProgress), `status` |
|  | `PocketEntry` | `transRefId`, `stepOrder`, `debit` (pocketId), `credit` (pocketId), `amount` — **1 dòng \= 1 bút toán** |
...
| **Runtime / Biên lai** | `TransactionTrail` | `id` (= `transRefId`), `service`, `inputMessage`, `outputMessage` (TRANSBODY), `transStepLog[]`, `status` (init/pending/done/failed) |
|  | `Transaction` | `code`, `transRefId`, `service`, `sender`, `receiver`, `amount`, `fee`, `totalAmount`, `status`, `billerRefId` |

**4 loại ví (Pocket):** Customer & Biller tự sinh theo chủ thể; System (gom phí) & Bank (nguồn cash-in) do Officer tạo tay. **FK config** \= `String(service._id)`. **Tiền chỉ chạy ở Verify**, trong `session.withTransaction`.
