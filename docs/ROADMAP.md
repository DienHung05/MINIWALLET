...
|----|----------|----------|
| P0 | Auth customer theo MINIWALLET | Đã làm: customer đăng ký bằng `name/phone/pin`; đăng nhập bằng số điện thoại + PIN; admin dùng tài khoản mặc định đã seed. |
| P1 | Quên/đặt lại PIN | Đã làm API + UI `forgot-pin` và `reset-pin`; dev/MVP có thể trả reset token trong response để demo. |
| P2 | Design system + AppShell | Đã làm shell customer/admin, component UI cơ bản, responsive mobile. |
| P3 | Customer UX hoàn chỉnh | Đã làm dashboard ví theo nghiệp vụ lõi, P2P, Bill Payment, lịch sử; extension chuyển liên ngân hàng, liên kết bank/card, topup tách riêng. |
| P4 | Admin UX hoàn chỉnh | Đã làm tổng quan, Service, Transaction Design, Ví, Biller, Customer, Cash-in, Trail, History, đối soát, connector, công cụ kỹ thuật. |
| P5 | Hoàn thiện nghiệp vụ theo MINIWALLET | Đã làm P2P, CASH_IN, BILL_PAYMENT, mock Biller, Biller/Wallet/Customer API; các extension D1/D2/B4/B5 vẫn giữ. |
| P6 | Hardening & QA | Đã có `backend npm run qa:engine`: test phone/PIN auth, Cash-in, Bill Payment, verify replay, callback idempotency SUCCESS/FAILED, timeout reversal, checksum và bảo toàn tổng số dư; còn manual e2e/production readiness nếu cần. |
